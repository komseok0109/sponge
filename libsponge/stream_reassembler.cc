...
void StreamReassembler::push_substring(const string &data, const size_t index, const bool eof) {
    ...
    if (index < first_unacceptable && index + data.size() > first_unassembled) {
        const size_t start = max(index, first_unassembled); //!< Start index of acceptable byte in 'data' w.r.t the whole stream.
        const size_t end = min(index + data.size(), first_unacceptable); //!< End index of acceptable byte in 'data' + 1 w.r.t the whole stream
